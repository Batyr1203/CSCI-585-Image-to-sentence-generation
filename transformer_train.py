...
f = open('train_logs_transformers/train_coco_log_2.txt', 'a')
...
def validate(args, val_loader, encoder, decoder, criterion):
    ...
    metrics = {}
    weights = (1.0 / 1.0,)
    metrics["Bleu_1"] = corpus_bleu(references, hypotheses, weights)
    weights = (1.0/2.0, 1.0/2.0,)
    metrics["Bleu_2"] = corpus_bleu(references, hypotheses, weights)
    weights = (1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0,)
    metrics["Bleu_3"] = corpus_bleu(references, hypotheses, weights)
    ...
    f.write("EVA LOSS: {} TOP-5 Accuracy {} BLEU-1 {} BLEU2 {} BLEU3 {} BLEU-4 {}".format
          (losses.avg, top5accs.avg,  metrics["Bleu_1"],  metrics["Bleu_2"],  metrics["Bleu_3"],  metrics["Bleu_4"]))
...
if __name__ == '__main__':
    ...
    # Data parameters
    parser.add_argument('--data_folder', default="/workspace/datasets/COCO_2014/out/",
    ...
    parser.add_argument('--embedding_path', default='/workspace/datasets/GloveWordEmbedding/glove.6B.300d.txt', help='path to pre-trained word Embedding.')
    ...
    if args.checkpoint is None:
        ...
        if args.embedding_path is not None:
            ...
            f.write('Loaded {} pre-trained word embeddings.\n'.format(len(word_embeds)))
        ...
        if args.fine_tune_encoder is True and encoder_optimizer is None:
            ...
            f.write("Encoder_Optimizer is None, Creating new Encoder_Optimizer!\n")
            encoder.fine_tune(args.fine_tune_encoder)
            encoder_optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, encoder.parameters()),
                                                 lr=args.encoder_lr)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epochs_since_improvement == args.stop_criteria:
            ...
            f.write("the model has not improved in the last {} epochs\n".format(args.stop_criteria))
            break
